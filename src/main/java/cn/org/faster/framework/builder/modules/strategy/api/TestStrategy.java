...
public class TestStrategy extends BuildStrategy {
    ...
    public void process(ZipOutputStream zipOutputStream) throws IOException {
        ...
        srcPath = BuilderConstants.JAVA_TEST_PATH + builderModel.getBasePath() + "/test/" + modulesName + "/v1/";
        String basePackage = builderModel.getBasePackagePath() + ".test." + modulesName + ".v1";
        for (TableColumnModel tableColumnModel : columnModelList) {
            ...
            StringBuffer importStr = new StringBuffer()
                    ...
                    .append("import ").append(builderModel.getBasePackagePath()).append("." + modulesName + ".v1").append(".").append(tableColumnModel.getBusinessEnName()).append(".model.request.").append(tableColumnModel.getBusinessEnNameUpFirst()).append("AddRequest;\n")
                    .append("import ").append(builderModel.getBasePackagePath()).append("." + modulesName + ".v1").append(".").append(tableColumnModel.getBusinessEnName()).append(".model.request.").append(tableColumnModel.getBusinessEnNameUpFirst()).append("UpdateRequest;\n");
